...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/analyst_upgrades_today \
  -H 'Content-Type: application/json' \
  -d '{"action":"upgrade","limit":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/analyst_upgrades_today`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
